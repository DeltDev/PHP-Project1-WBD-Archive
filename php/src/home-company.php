...
<body method="get">
    <header>
        <div class="nav">
            <div class="hyperlink">
                <a href="profil-company.php">
                    <img src="public/assets/images/perusahaanAnda.svg" alt="Perusahaan Anda">
                </a>
    ...
    <main id="main">
            ...
            <content id="konten">
                <?php for ($i = 0; $i < count($lowonganList); $i++) :?>
                    <article class="card" id=<?= $lowonganList[$i]["lowongan_id"];?>>
                        ...
                        <a class="detail" href="vacancy-details-company.php?lowongan_id=<?= $lowonganList[$i]["lowongan_id"];?>">Detail</a>

                        <!-- Tombol Edit -->
                        <form id="editForm-<?= $lowonganList[$i]['lowongan_id']; ?>" action="set-session.php" method="post">
                            <input type="hidden" name="lowongan_id" value="<?= $lowonganList[$i]['lowongan_id']; ?>">
                        </form>
                        <a class="edit" href="javascript:void(0)" onclick="submitForm('editForm-<?= $lowonganList[$i]['lowongan_id']; ?>')">
                            <img src="public/assets/images/edit.svg" alt="Edit">
                        </a>

                        <!-- Tombol Hapus -->
                        <form id="deleteForm-<?= $lowonganList[$i]['lowongan_id']; ?>" action="set-session.php" method="post">
                            <input type="hidden" name="hapus" value="<?= $lowonganList[$i]['lowongan_id']; ?>">
                        </form>
                        <a class="trash" href="javascript:void(0)" onclick="confirmDelete('<?= $lowonganList[$i]['lowongan_id']; ?>')">
        ...
        <?php endif ?>
        <a href="create-vacancy.php" class="tambah">
            <img src="public/assets/images/tambah.svg" alt="tambah">
        </a>
    </main>
